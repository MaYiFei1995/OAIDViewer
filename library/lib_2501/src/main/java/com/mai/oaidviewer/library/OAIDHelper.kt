...
class OAIDHelper(context: Context, callback: OAIDCallback) : OAIDImpl(context, callback) {
    ...
    override fun init() {
        ...
        Thread {
            ...
            val listener = object : IIdentifierListener {
            ...
            try {
                // if x86 throws Error
                // MdidSdkHelper.InitSdk(cxt, isSDKLogOn, isGetOAID, isGetVAID, isGetAAID, listener);
                val ret = MdidSdkHelper.InitSdk(context, true, true, true, true, listener)
    ...
    override fun getSdkVersion(): Pair<String, String> {
        return Pair("2.5.1", MdidSdkHelper.SDK_VERSION_CODE.toString())
    }
